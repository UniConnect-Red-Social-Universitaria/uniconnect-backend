...
import {
  ...
  validarCorreoConGoogle,
} from "../utils/registro.util";
...
export class UsuarioController {
  ...
  static async buscarPorMateria(req: Request, res: Response) {
    try {
      ...
      const idsRelacionados = await ContactoModel.obtenerIdsRelacionados(
        req.usuario.id,
      );
      const resultados = await UsuarioModel.buscarPorMateriaExcluyendo(
        materia,
        req.usuario.id,
        idsRelacionados,
      );
  ...
  static async enviarSolicitudConexion(req: Request, res: Response) {
    try {
      ...
      const relacionExistente = await ContactoModel.existeRelacionEntreUsuarios(
        req.usuario.id,
        usuarioDestinoId,
      );
      ...
      const solicitud = await ContactoModel.crearSolicitud(
        req.usuario.id,
        usuarioDestinoId,
      );
  ...
  static async listarCompaneros(req: Request, res: Response) {
    try {
      ...
      const companeros = await ContactoModel.listarCompanerosAceptados(
        req.usuario.id,
      );
  ...
  static async listarSolicitudesRecibidas(req: Request, res: Response) {
    try {
      ...
      const solicitudes = await ContactoModel.listarSolicitudesRecibidas(
        req.usuario.id,
      );
  ...
  static async aceptarSolicitud(req: Request, res: Response) {
    try {
      ...
      const solicitudActualizada = await ContactoModel.aceptarSolicitud(
        solicitudId,
        req.usuario.id,
      );
  ...
  static async registrar(req: Request, res: Response) {
    try {
      ...
      const materiasNormalizadas = materiasCursando.map((materia) =>
        materia.trim(),
      );
      ...
      if (cantidadCarreras > 0) {
        carreraCatalogo =
          await CarreraModel.buscarPorNombre(carreraNormalizada);
      ...
      if (cantidadMaterias > 0) {
        ...
        const setMaterias = new Set(
          materiasCatalogo.map((materia) => materia.nombre.toLowerCase()),
        );
      ...
      if (
        ...
        (error.message.includes("Google") || error.message.includes("correo"))
      ) {
        return res.status(400).json({
          success: false,
          message: error.message,
  ...
  static async login(req: Request, res: Response) {
    try {
      ...
      const usuario = await UsuarioModel.buscarPorCorreoConContrasena(
      ...
      const contrasenaValida = await bcrypt.compare(
        contrasena,
        usuario.contrasenaHash as string,
      );
  ...
  static async actualizarPerfil(req: Request, res: Response) {
    try {
      ...
      const setMaterias = new Set(
        materiaCatalogo.map((materia) => materia.nombre.toLowerCase()),
      );
      ...
      if (typeof carrera === "string" && cantidadCarreras > 0) {
        const carreraCatalogo = await CarreraModel.buscarPorNombre(
          carrera.trim(),
        );
      ...
      if (
        semestre !== undefined &&
        (!Number.isInteger(semestre) || semestre <= 0)
      ) {
        return res.status(400).json({
          success: false,
      ...
      const usuarioDinamico = usuarioActualizado as unknown as Record<
        string,
        unknown
      >;
